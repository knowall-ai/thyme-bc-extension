...
=== Current User API (Page 50106)

Returns the currently authenticated user's BC information. Useful for matching the logged-in user to their Resource record.

Maps to the BC `User` table (virtual table 2000000120). Filtered to current user via `UserSecurityId()`.
...
| Thyme Current User API | Page 50106
